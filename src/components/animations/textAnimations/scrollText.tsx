...
const creativity = 'Building ';
const is = 'the ';
const my = 'web ';
const craft = 'together';
const sentence3 = 'innovate create inspire';
...
const animateLettersOnScroll = (containerRef: MutableRefObject<any>) => {
  ...
  letterElements.forEach((letter: Element, index: number) => {
    // Generate random speed here instead of in render
    const randomSpeed = 0.8 + Math.random() * (1.5 - 0.8);

    gsap.to(letter, {
      y: () => (1 - randomSpeed) * ScrollTrigger.maxScroll(window),
      ease: 'power2.out',
      duration: 0.8,
      scrollTrigger: {
